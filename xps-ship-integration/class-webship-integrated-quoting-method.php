...
class Webship_Integrated_Quoting_Method extends WC_Shipping_Method {
	...
	public function __construct() {
		...
		$this->method_title = esc_html( sprintf( __( '%s Integrated Quoting', 'xpsship-integration' ), $GLOBALS['client_info']['clientName'] ) );
		// Translators: Sub the clientName into the description.
		$this->method_description = esc_html( sprintf( __( 'Integrated Quoting from %s', 'xpsship-integration' ), $GLOBALS['client_info']['clientName'] ) );
	...
	public function admin_options() {
		...
		echo wp_kses(
			...
			. '<tr><th></th><td><i>Enable and configure your ability to load orders and fulfill them from ' . esc_html( $GLOBALS['client_info']['clientName'] ) . ' on the <a href="' . $permalink . '">' . esc_html( $GLOBALS['client_info']['clientName'] ) . ' settings page</a></i></td></tr>'
			. '</table>',
			$GLOBALS['allowed_html']
		);
	}
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled' => array(
				'title'       => esc_attr( __( 'Enable', 'xpsship-integration' ) ),
				'type'        => 'checkbox',
				'description' => esc_attr( __( 'Enable Integrated Quoting', 'xpsship-integration' ) ),
				'default'     => 'yes',
			),

			'apiKey'  => array(
				'title'       => esc_attr( __( 'API Key', 'xpsship-integration' ) ),
				'type'        => 'text',
				// Translators: sub the clientName into the description.
				'description' => esc_html( sprintf( __( 'Your WooCommerce Integrated Quoting API Key generated by %s', 'xpsship-integration' ), $GLOBALS['client_info']['clientName'] ) ),
				'default'     => '',
			),
		);
		...
		if ( ! webship_client_has_dedicated_plugin() ) {
			$this->form_fields['url'] = array(
				// Translators: sub the clientName into the title.
				'title'       => esc_html( sprintf( __( '%s URL', 'xpsship-integration' ), $GLOBALS['client_info']['clientName'] ) ),
				'type'        => 'text',
				// Translators: sub the clientName into the description.
				'description' => esc_html( sprintf( __( 'Your %s Base URL', 'xpsship-integration' ), $GLOBALS['client_info']['clientName'] ) ),
				'default'     => '',
			);
		}
	}
